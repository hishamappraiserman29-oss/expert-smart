...
class TestReportsList:
    def test_HI01_returns_200(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_LIST_RESULT):
            r = client.get("/api/reports")
        assert r.status_code == 200

    def test_HI02_status_is_success(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_LIST_RESULT):
            data = client.get("/api/reports").get_json()
        assert data["status"] == "success"

    def test_HI03_response_has_count(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_LIST_RESULT):
            data = client.get("/api/reports").get_json()
    ...
    def test_HI04_response_has_reports_list(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_LIST_RESULT):
            data = client.get("/api/reports").get_json()
...
class TestPagination:
    def test_HI05_limit_param_forwarded(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_EMPTY_RESULT) as mock_fr:
            client.get("/api/reports?limit=5")
        kw = mock_fr.call_args.kwargs
        assert kw.get("limit") == 5

    def test_HI06_offset_param_forwarded(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_EMPTY_RESULT) as mock_fr:
            client.get("/api/reports?offset=10")
        kw = mock_fr.call_args.kwargs
        assert kw.get("offset") == 10

    def test_HI06b_limit_capped_at_100(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_EMPTY_RESULT) as mock_fr:
            client.get("/api/reports?limit=999")
        kw = mock_fr.call_args.kwargs
        assert kw.get("limit") == 100

    def test_HI06c_invalid_limit_defaults_to_20(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_EMPTY_RESULT) as mock_fr:
            client.get("/api/reports?limit=notanint")
        kw = mock_fr.call_args.kwargs
...
class TestFilters:
    def test_HI07_profile_key_filter_forwarded(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_EMPTY_RESULT) as mock_fr:
            client.get("/api/reports?profile_key=detailed")
        kw = mock_fr.call_args.kwargs
        assert kw.get("profile_key") == "detailed"

    def test_HI07b_professional_template_profile_forwarded(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_EMPTY_RESULT) as mock_fr:
            client.get("/api/reports?profile_key=professional_template")
        kw = mock_fr.call_args.kwargs
        assert kw.get("profile_key") == "professional_template"

    def test_HI08_status_filter_forwarded(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_EMPTY_RESULT) as mock_fr:
            client.get("/api/reports?status=final")
        kw = mock_fr.call_args.kwargs
        assert kw.get("status") == "final"

    def test_HI08b_absent_filters_are_none(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   return_value=_EMPTY_RESULT) as mock_fr:
...
class TestReportsGet:
    def test_HI09_found_returns_200(self, client):
        with patch("reports.report_pipeline.fetch_report",
                   return_value=_FULL_RECORD):
            r = client.get("/api/reports/ES-20260515-AAA1")
        assert r.status_code == 200

    def test_HI09b_found_status_success(self, client):
        with patch("reports.report_pipeline.fetch_report",
                   return_value=_FULL_RECORD):
            data = client.get("/api/reports/ES-20260515-AAA1").get_json()
        assert data["status"] == "success"

    def test_HI09c_found_report_key_present(self, client):
        with patch("reports.report_pipeline.fetch_report",
                   return_value=_FULL_RECORD):
            data = client.get("/api/reports/ES-20260515-AAA1").get_json()
    ...
    def test_HI09d_found_report_has_full_dto_fields(self, client):
        with patch("reports.report_pipeline.fetch_report",
                   return_value=_FULL_RECORD):
            data = client.get("/api/reports/ES-20260515-AAA1").get_json()
        report = data["report"]
...
class TestReportsGetNotFound:
    def test_HI10_missing_returns_404(self, client):
        with patch("reports.report_pipeline.fetch_report", return_value=None):
            r = client.get("/api/reports/NONEXISTENT-ID")
        assert r.status_code == 404

    def test_HI11_missing_status_is_not_found(self, client):
        with patch("reports.report_pipeline.fetch_report", return_value=None):
            data = client.get("/api/reports/NONEXISTENT-ID").get_json()
        assert data["status"] == "not_found"

    def test_HI11b_missing_message_contains_id(self, client):
        with patch("reports.report_pipeline.fetch_report", return_value=None):
            data = client.get("/api/reports/NONEXISTENT-ID").get_json()
...
class TestDBExceptions:
    def test_HI12_list_db_error_returns_500(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   side_effect=RuntimeError("disk full")):
            r = client.get("/api/reports")
        assert r.status_code == 500

    def test_HI12b_list_db_error_status_is_error(self, client):
        with patch("reports.report_pipeline.fetch_reports",
                   side_effect=RuntimeError("disk full")):
            data = client.get("/api/reports").get_json()
    ...
    def test_HI13_get_db_error_returns_500(self, client):
        with patch("reports.report_pipeline.fetch_report",
                   side_effect=RuntimeError("connection timeout")):
            r = client.get("/api/reports/ES-20260515-AAA1")
        assert r.status_code == 500

    def test_HI13b_get_db_error_status_is_error(self, client):
        with patch("reports.report_pipeline.fetch_report",
                   side_effect=RuntimeError("connection timeout")):
            data = client.get("/api/reports/ES-20260515-AAA1").get_json()
        assert data["status"] == "error"
...
class TestNoProductionDB:
    def test_HI14_no_db_file_created_in_list(self, client, tmp_path):
    ...
    def test_HI14b_no_db_file_created_in_get(self, client):
        """Mocking fetch_report means the real DB is never touched."""
        prod_db = Path(_CORE) / "reports" / "db" / "data" / "reports.db"
        existed_before = prod_db.exists()
        with patch("reports.report_pipeline.fetch_report", return_value=None):
            client.get("/api/reports/any-id")
        if not existed_before:
            assert not prod_db.exists()
