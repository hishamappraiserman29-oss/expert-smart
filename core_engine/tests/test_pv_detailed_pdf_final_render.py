# -*- coding: utf-8 -*-
"""test_pv_detailed_pdf_final_render.py — Detailed PDF Final Render Gate.

Verifies that the Detailed tier PDF was generated via Playwright/Chromium,
contains all 38 logical sections, passes all governance checks, and that
no Playwright-related test is skipped.

Run:
    .venv/Scripts/python.exe -m pytest core_engine/tests/test_pv_detailed_pdf_final_render.py -q
"""
...
GATE   = CORE / "instance" / "manual_review_outputs" / \
         "professional_valuation_detailed_final_render_gate"
PDF    = GATE / "actual_file" / "FINAL_DETAILED_REPORT.pdf"
AUDITS = GATE / "audits"
VISUAL_INDEX = GATE / "visual_index" / "OPEN_FINAL_DETAILED_REPORT_REVIEW.html"

BATCH_START = "2026-07-13T00:00:00Z"

REQUIRED_SECTION_IDS = [
    "sec-cover", "sec-advisory", "sec-exec-summary", "sec-facts",
    "sec-sources", "sec-data-quality", "sec-market-evidence", "sec-avm",
    "sec-comparables", "sec-sales-adj", "sec-land-sales", "sec-land-adj",
    "sec-land-extraction", "sec-land-residual", "sec-land-recon",
    "sec-cost", "sec-boq", "sec-rcn", "sec-depreciation",
    "sec-site-improvements", "sec-income-cap", "sec-rent-comps",
    "sec-sale-rent", "sec-dcf", "sec-npv-irr", "sec-sensitivity",
    "sec-scenarios", "sec-risk-register", "sec-risk-matrix", "sec-esg",
    "sec-hbu", "sec-swot", "sec-reconciliation", "sec-conclusion",
    "sec-recommendation", "sec-nextdocs", "sec-qdrant", "sec-signature",
]
...
SAMPLE_DATA = {
    "report_title":      "تقرير تقييم عقاري آلي — تفصيلي (Detailed)",
...
def test_D01_correct_venv_used():
...
def test_D02_playwright_imports():
    """playwright.sync_api is importable."""
    from playwright.sync_api import sync_playwright  # noqa: F401


def test_D03_chromium_available():
...
def test_D05_final_pdf_exists():
    """FINAL_DETAILED_REPORT.pdf exists in actual_file/."""
    assert PDF.exists(), f"Final PDF missing: {PDF}"


def test_D06_pdf_generated_after_batch_start():
...
def test_D07_playwright_renderer_used():
    """Generation audit confirms Playwright/Chromium renderer."""
    audit = json.loads((AUDITS / "04_fresh_detailed_pdf_generation.json").read_text(encoding="utf-8"))
    assert audit.get("playwright_used") is True
    assert audit.get("fpdf_used") is False


def test_D08_fpdf_not_imported_in_builder():
    """fpdf2 is never imported in the PDF builder (comment 'No FPDF' is allowed)."""
...
def test_D09_pdf_opens_successfully():
...
def test_D10_actual_page_count_from_pdf():
    """PDF has at least 14 pages (Detailed is richer than Traditional)."""
    import fitz
    doc = fitz.open(str(PDF))
    count = doc.page_count
    doc.close()
    assert count >= 14, f"Too few pages: {count}"


# ── PNG rendering ──────────────────────────────────────────────────────────────

def test_D11_every_pdf_page_has_png():
...
def test_D14_no_critical_visual_defects():
    """Visual defects audit reports zero critical defects."""
    audit = json.loads((AUDITS / "08_detailed_visual_defects.json").read_text(encoding="utf-8"))
    assert audit["critical_count"] == 0, f"Critical defects: {audit['defects']}"


# ── Required disclaimers ───────────────────────────────────────────────────────

def test_D15_required_disclaimers_in_html():
...
def test_D16_signature_gate_unsigned():
...
def test_D17_no_automatic_certification():
...
def test_D18_no_fake_licence_stamp_signature():
...
def test_D19_no_internal_absolute_path():
...
def test_D20_no_valuation_recalculation_in_template():
    """Template does not import valuation engine modules."""
    template_text = (CORE / "templates" / "pdf" / "pv_detailed_report.html").read_text(encoding="utf-8")
...
def test_D21_visual_index_exists():
    ...
    assert "FINAL_DETAILED_REPORT.pdf" in content
    assert "detailed_page_001.png" in content
