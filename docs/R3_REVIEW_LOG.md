...
### Merged to main (all 24 subsystems + E2E bundle + mobile/ + CI fix)
...
| **mobile/** | **mobile gate** | **5e996f5** |

### Deferred

*None.* All R3 subsystems are on main.
...
- [x] mobile/ extraction path decision — ✅ RESOLVED (merged to main monorepo, `mobile/` directory)
- [ ] `GOVT_SIGNING_KEY` added to production PROD_READINESS_CHECKLIST.md (Section 1: Security)
- [ ] B3 (R3.11): tenant_id ↔ owner_user_id bridge — P2, non-blocking
- [ ] **PH.3 closure** — GCP service account key rotation/delete/disable; blocks Full Production GO (waiver expires 2026-06-19)
- [ ] Mobile CI workflow — add `.github/workflows/mobile.yml` (P3)

### WIP branch fate: KEEP AS ARCHIVE (recommended)

`wip/r3-subsystems-checkpoint` preserves the original checkpoint. All 24 subsystems,
the E2E bundle, and `mobile/` are now on main. The WIP branch has no remaining
open items and may be archived or deleted at the owner's discretion.
