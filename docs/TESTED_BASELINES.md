...
| 3c | `POST /api/valuation` — التجمع الخامس apartment | 200 | ✅ Pass (Docker HTTP-confirmed, April 30, 2026 — reproduces 3b) | `market_value = 6,358,000`, AVM `medium / n=16` |
| 3d | `POST /api/valuation` — العاصمة الإدارية apartment | 200 | ✅ Pass (Docker HTTP-confirmed, April 30, 2026 — first **high** tier) | `market_value = 5,731,740`, AVM `high / n=68` |
| 3e | `POST /api/valuation` — مدينة نصر commercial | 200 | ✅ Pass (Docker HTTP-confirmed, April 30, 2026) | `market_value = 3,711,150`, AVM `medium / n=26` |
...
#### Baseline 3c–3e — Local Docker valuation baseline pack (April 30, 2026)

Three additional `/api/valuation` runs HTTP-confirmed against the local Docker stack. Together they re-confirm 3b in the containerized stack, activate the AVM "high" tier for the first time in this project, and exercise the commercial dispatch path.

| sub-baseline | location | property_type | area | market_value | avm.applied | avm.confidence | avm.n_records | avm.avm_ppm |
|---|---|---|---:|---:|---|---|---:|---:|
| **3c** | التجمع الخامس | شقة سكنية | 200 | 6,358,000 | true | medium | 16 | 32,872 |
| **3d** | العاصمة الإدارية | شقة سكنية | 180 | 5,731,740 | true | **high** | 68 | 34,335 |
| **3e** | مدينة نصر | تجاري | 150 | 3,711,150 | true | medium | 26 | 25,776 |

All three calls used the same minimum payload shape (`location`, `property_type`, `valuation_purpose="fair_market_value"`, `area`). No `price_per_meter` supplied — AVM auto-injection in every case.

**Verdicts (Arabic, as returned):**
- 3c: تم اعتماد سعر المتر من AVM = 32,872 ج.م (confidence=medium, n=16 معاملة).
- 3d: تم اعتماد سعر المتر من AVM = 34,335 ج.م (confidence=high, n=68 معاملة).
- 3e: تم اعتماد سعر المتر من AVM = 25,776 ج.م (confidence=medium, n=26 معاملة).

For all three: `status="success"`, `excel_url` populated, `purpose_report_url` populated (`PurposeReport_fair_market_value_*.docx`).

**Notes:**
- **3c** reproduces 3b's `market_value` (6,358,000) exactly in Docker. Confirms the `templates/template.xlsm` COPY fix and the `waitress` dependency reached the container correctly.
- **3d** is the first time the AVM "high" confidence tier (n≥30 AND time_span≥6 months) activates in this project. `weight_uplift=1.00` vs. 0.50 for medium.
- **3e** confirms `/api/valuation` dispatches correctly for commercial property types (`property_type="تجاري"`).

**Acceptance window for 3c–3e:** `status="success"`; `avm.applied=true`; `market_value` within ±5% of the values above; `excel_url` and `purpose_report_url` both non-empty.
...
- Tests run on: April 25, 2026 (initial validation + AVM densification follow-up); April 30, 2026 (Docker valuation baseline pack — Baselines 3c–3e)
...
- **Docker HTTP-level confirmation (April 30, 2026):** Baselines 3c–3e were invoked from PowerShell against the local Docker stack (`docker compose -f deploy/docker-compose.yml up -d`), with Flask served by waitress inside the `expert-smart-flask` container, Ollama and Qdrant healthy as sidecars. Numbers are real HTTP response values.
